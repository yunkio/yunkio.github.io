...
여기서 $\lim N \rightarrow \infin$ 를 가정했습니다. 비슷하게 $Y$값과 무관하게 $X$가 $x_i$값을 가질 확률을 $p(X = x_i)$로 적을 수 있습니다.
...
$X = x_i$인 사례들만 고려해보겠습니다. 이 중에서 $Y=y_{ij}$인 사례들의 비율을 생각해 볼 수 있으며 이를 확률 $p(Y=y_j|X = x_i)$로 적을 수 있습니다. 이를 **조건부 확률** 이라고 부릅니다. 
